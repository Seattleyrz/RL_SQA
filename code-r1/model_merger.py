...
# Example usage:
# python model_merger.py --backend fsdp --local_dir "./models/apps-ppo/global_step_32/actor" --hf_upload_path "JeffZ0930/apps-ppo-32" --token your_token
...
parser.add_argument("--hf_model_path", type=str, default="Qwen/Qwen2.5-Coder-3B-Instruct", help="The path for the huggingface model")
parser.add_argument(
    "--local_dir",
    type=str,
    required=True,
...
def upload_model_to_huggingface(hf_path, token):
    # Push to hugging face
    from huggingface_hub import HfApi

    api = HfApi()
    api.create_repo(repo_id=args.hf_upload_path, private=False, exist_ok=True, token=token)
    api.upload_folder(folder_path=hf_path, repo_id=args.hf_upload_path, token=token, repo_type="model")
...
def convert_fsdp_checkpoints_to_hfmodels():
    ...
    if args.hf_upload_path:
        upload_model_to_huggingface(hf_path, token)
